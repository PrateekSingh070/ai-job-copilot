...
// The model returns JSON as text, sometimes wrapped in prose or a code fence,
// so we pull out the first JSON object and validate it against the schema.
...
function sanitizeResumeHtml(input) {
...
export async function saveGeneration(input) {
  ...
  const saved = await prisma.aiGeneration.create({
    data: {
      userId: input.userId,
      type: input.type,
      inputJson: input.payloadIn,
      outputJson: input.payloadOut,
      model: input.model,
