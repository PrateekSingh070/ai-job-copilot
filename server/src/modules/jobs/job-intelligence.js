...
export function extractTopKeywords(text, take = 20) {
...
export function computeFitScore(resumeText, jobDescription) {
...
export function runAtsChecks(resumeText, jobDescription) {
...
function inferCompanySize(input) {
  const text = input.toLowerCase();
...
export function computeApplicationReminders(jobs) {
  const now = Date.now();
